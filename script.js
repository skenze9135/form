...
function setupRealtimeListeners() {
    ...
    db.ref('scans').orderByKey().limitToLast(1).on('value', (snapshot) => {
        ...
        if (data) {
            ...
            updateDashboard(latestScan, latestScanKey);
        }
    });
...
function updateDashboard(scanData, scanId = null) {
    // Update device name
    ...
    // Update Scan Time Header
    if (scanId) {
        const timestamp = getTimestampFromId(scanId);
        const date = new Date(timestamp);
        document.getElementById('lastUpdate').textContent = `Scan Time: ${date.toLocaleString()}`;
    }
    ...
    if (currentNetworks.length > 0) {
        ...
        let strongestName = strongest.ssid;
        if (!strongestName && strongest.hidden) strongestName = '<Hidden>';
        else if (!strongestName) strongestName = 'Unknown';
